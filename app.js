...
Devuelve la propuesta en formato JSON con la explicación breve de tus cálculos.`;
  ...
  const runLocalAnalysis = (eLog, fLog, mLog, nts, tgt) => {
    ...
    setTdeeEstimate(tdee);
    if (tdee && tgt) {
      // ⚡ Bolt: Use O(N) reduce instead of O(N log N) sort to find the latest metric
      const latestM = Object.entries(mLog||{}).reduce((latest, current) => {
        if (!current[1]?.weight) return latest;
...
No repitas los datos que ya te mandé. No me pidas registrar nada.`;
  ...
  const generateWeeklyPDF = async () => {
    ...
    try {
      ...
      // ⚡ Bolt: Use O(N) reduce instead of O(N log N) sort to find the latest metric date
...
function Coach({
  ...
  const contextSummary = React.useMemo(() => {
    ...
    // ⚡ Bolt: Use O(N) reduce instead of O(N log N) sort to find the latest metrics entry
    const latestMetrics = Object.entries(metricslog || {}).reduce((latest, current) => {
      if (!latest) return current;
      return current[0] > latest[0] ? current : latest;
    }, undefined);
...
function FocusMode({ onClose, splits, exlog, exercises }) {
  ...
  const getLastEntry = (exName) => {
    ...
    // ⚡ Bolt: Use O(N) reduce instead of O(N log N) sort to find the latest entry
    const last = work.reduce((latest, current) => {
      if (!latest) return current;
