...
Devuelve la propuesta en formato JSON con la explicación breve de tus cálculos.`;
  ...
  const runLocalAnalysis = (eLog, fLog, mLog, nts, tgt) => {
    ...
    if (tdee && tgt) {
      ...
      const latestM = Object.entries(mLog||{}).reduce((max, curr) => (curr[1]?.weight && (!max || curr[0] > max[0])) ? curr : max, null)?.[1];
...
No repitas los datos que ya te mandé. No me pidas registrar nada.`;
  ...
  const generateWeeklyPDF = async () => {
    ...
    try {
      ...
      const latestMetricDate = Object.keys(metricslog||{}).reduce((max, curr) => (!max || curr > max) ? curr : max, null);
...
function Coach({
  ...
  const contextSummary = React.useMemo(() => {
    ...
    // ⚡ Bolt: Replaced O(N log N) sort with O(N) reduce to find latest metrics
    const latestMetrics = Object.entries(metricslog || {}).reduce((max, curr) => (!max || curr[0] > max[0]) ? curr : max, null);
...
function FocusMode({ onClose, splits, exlog, exercises }) {
  ...
  const getLastEntry = (exName) => {
    ...
    // ⚡ Bolt: Replaced O(N log N) sort with O(N) reduce to find latest workout entry
    const last = work.reduce((max, curr) => (!max || curr.date > max.date) ? curr : max, work[0]);
