...
Devuelve la propuesta en formato JSON con la explicación breve de tus cálculos.`;
  ...
  const runLocalAnalysis = (eLog, fLog, mLog, nts, tgt) => {
    ...
    setTdeeEstimate(tdee);
    if (tdee && tgt) {
      const latestM = Object.entries(mLog||{}).filter(([_,v])=>v?.weight).reduce((max, curr) => (!max || curr[0] > max[0]) ? curr : max, undefined)?.[1]; // Bolt: O(N) max search
...
No repitas los datos que ya te mandé. No me pidas registrar nada.`;
  ...
  const generateWeeklyPDF = async () => {
    ...
    try {
      ...
      const latestMetricDate = Object.keys(metricslog||{}).reduce((max, curr) => (!max || curr > max) ? curr : max, undefined); // Bolt: O(N) max search
