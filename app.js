...
function detectPlateaus(exlog) {
  ...
  Object.entries(exlog||{}).forEach(([exName, sets])=>{
    ...
    // Bolt: Use lexicographical string comparison for faster date sorting
...
function calcProgressiveOverload(exlog) {
  ...
  Object.entries(exlog||{}).forEach(([exName, sets])=>{
    ...
    // Bolt: Use lexicographical string comparison for faster date sorting (descending)
    const sorted = [...sets].filter(s=>s?.date&&s?.w).sort((a,b) => a.date > b.date ? -1 : (a.date < b.date ? 1 : 0));
...
Devuelve la propuesta en formato JSON con la explicación breve de tus cálculos.`;
  ...
  const runLocalAnalysis = (eLog, fLog, mLog, nts, tgt) => {
    ...
    if (tdee && tgt) {
      // Bolt: Use lexicographical string comparison for faster date sorting (descending)
      const latestM = Object.entries(mLog||{}).filter(([_,v])=>v?.weight).sort((a,b)=>a[0] > b[0] ? -1 : (a[0] < b[0] ? 1 : 0))[0]?.[1];
  ...
  const buildWorkoutHistorySummary = () => {
    ...
    const progressLines = Object.entries(recentByEx)
      .map(([ex, sets]) => {
        // Bolt: Use lexicographical string comparison for faster date sorting (ascending)
