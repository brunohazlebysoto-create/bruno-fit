...
export default function App(){
  ...
  const getMetricsForDate = (dateStr) => {
    // Bolt: Replaced O(N log N) filter+sort with O(N) reduce to safely find latest entry
    const latestEntry = Object.entries(metricslog || {}).reduce((latest, current) => {
      if (current[0] <= dateStr && (!latest || current[0] > latest[0])) {
        return current;
      }
      return latest;
    }, undefined);
...
Devuelve la propuesta en formato JSON con la explicación breve de tus cálculos.`;
  ...
  const runLocalAnalysis = (eLog, fLog, mLog, nts, tgt) => {
    ...
    setTdeeEstimate(tdee);
    if (tdee && tgt) {
      // Bolt: Replaced O(N log N) filter+sort with O(N) reduce to find latest weight safely
      const latestM = Object.entries(mLog||{}).filter(([_,v])=>v?.weight).reduce((max, curr) => (!max || curr[0] > max[0]) ? curr : max, undefined)?.[1];
...
No repitas los datos que ya te mandé. No me pidas registrar nada.`;
  ...
  const generateWeeklyPDF = async () => {
    ...
    try {
      ...
      // Bolt: Replaced O(N log N) sort.reverse()[0] with O(N) reduce to find latest date safely
      const latestMetricDate = Object.keys(metricslog||{}).reduce((max, d) => (!max || d > max) ? d : max, undefined);
...
function Coach({
  ...
  const contextSummary = React.useMemo(() => {
    ...
    // Bolt: Replaced O(N log N) Object.entries sort with O(N) reduce to extract latest entry safely
    const latestMetrics = Object.entries(metricslog || {}).reduce((max, curr) => (!max || curr[0] > max[0]) ? curr : max, undefined);
...
function FocusMode({ onClose, splits, exlog, exercises }) {
  ...
  const getLastEntry = (exName) => {
    ...
    // Bolt: Replaced O(N log N) array sort with O(N) reduce to find the last exercise entry safely
    const last = work.reduce((max, curr) => (!max || curr.date > max.date) ? curr : max, undefined);
