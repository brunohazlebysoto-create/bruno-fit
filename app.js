  ...
  const sendCoachMessage = async (messageText, customChat = null) => {
    ...
    try {
      ...
      const getFitdaysCompositionText = () => {
        ...
        // Optimization: O(N) linear search instead of O(N log N) sort to find max date
...
No repitas los datos que ya te mandé. No me pidas registrar nada.`;
  ...
  const generateWeeklyPDF = async () => {
    ...
    try {
      ...
      const mKeys = Object.keys(metricslog || {});
      // Optimization: O(N) linear search instead of O(N log N) sort to find latest date
      const latestMetricDate = mKeys.length > 0 ? mKeys.reduce((a, b) => a > b ? a : b) : undefined;
...
Analiza la tendencia de peso y composición corporal, identifica si está progresando hacia su objetivo de definición, y da sugerencias ESPECÍFICAS de calorías, macros y frecuencia de entrenamiento basadas en estos datos reales.`;
  ...
  return (
    <div className="pop">
      ...
      {(() => {
        const entry = metricslog[selectedDateStr] || {};
        const prevFiltered = Object.keys(metricslog).filter(d => d < selectedDateStr);
        // Optimization: O(N) linear search instead of O(N log N) sort
        const prevDates = prevFiltered.length > 0 ? [prevFiltered.reduce((a, b) => a > b ? a : b)] : [];
