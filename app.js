...
Devuelve la propuesta en formato JSON con la explicación breve de tus cálculos.`;
  ...
  const runLocalAnalysis = (eLog, fLog, mLog, nts, tgt) => {
    ...
    setTdeeEstimate(tdee);
    if (tdee && tgt) {
      // ⚡ Bolt: Replace O(N log N) sort with O(N) reduce to find latest date
      const latestM = Object.entries(mLog||{})
        .filter(([_,v])=>v?.weight)
        .reduce((latest, current) => !latest || current[0] > latest[0] ? current : latest, undefined)?.[1];
...
No repitas los datos que ya te mandé. No me pidas registrar nada.`;
  ...
  const generateWeeklyPDF = async () => {
    ...
    try {
      ...
      // ⚡ Bolt: Replace O(N log N) sort with O(N) reduce to find latest date
      const latestMetricDate = Object.keys(metricslog||{}).reduce((latest, current) => !latest || current > latest ? current : latest, undefined);
...
function Coach({
  ...
  const contextSummary = React.useMemo(() => {
    ...
    // ⚡ Bolt: Replace O(N log N) sort with O(N) reduce to find latest metrics
    const latestMetrics = Object.entries(metricslog || {}).reduce((latest, current) => !latest || current[0] > latest[0] ? current : latest, undefined);
...
function FocusMode({ onClose, splits, exlog, exercises }) {
  ...
  const getLastEntry = (exName) => {
    ...
    // ⚡ Bolt: Replace O(N log N) sort with O(N) reduce to find latest date without creating array copies
    const last = work.reduce((latest, current) => !latest || current.date > latest.date ? current : latest, undefined);
