...
Devuelve la propuesta en formato JSON con la explicación breve de tus cálculos.`;
  ...
  const runLocalAnalysis = (eLog, fLog, mLog, nts, tgt) => {
    ...
    setTdeeEstimate(tdee);
    if (tdee && tgt) {
      // ⚡ Bolt: Replaced O(N log N) sort with O(N) reduce to find latest weight entry
      const latestDate = Object.keys(mLog||{}).reduce((max, key) => (mLog[key]?.weight && (!max || key > max) ? key : max), undefined);
      const latestM = latestDate ? mLog[latestDate] : undefined;
...
No repitas los datos que ya te mandé. No me pidas registrar nada.`;
  ...
  const generateWeeklyPDF = async () => {
    ...
    try {
      ...
      // ⚡ Bolt: Replaced O(N log N) sort.reverse() with O(N) reduce
      const latestMetricDate = Object.keys(metricslog||{}).reduce((max, key) => (!max || key > max ? key : max), undefined);
...
function Coach({
  ...
  const contextSummary = React.useMemo(() => {
    ...
    // ⚡ Bolt: Replaced O(N log N) Object.entries sort with O(N) reduce on keys
    const latestMetricsKey = Object.keys(metricslog || {}).reduce((max, key) => (!max || key > max ? key : max), undefined);
    const latestMetrics = latestMetricsKey ? [latestMetricsKey, metricslog[latestMetricsKey]] : undefined;
...
function FocusMode({ onClose, splits, exlog, exercises }) {
  ...
  const getLastEntry = (exName) => {
    ...
    // ⚡ Bolt: Replaced O(N log N) array copy and sort with O(N) reduce
