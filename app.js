...
export default function App(){
  ...
  const getMetricsForDate = (dateStr) => {
    ...
    if (entries.length > 0) {
      // ⚡ Bolt: Use O(N) reduce instead of O(N log N) sort to find latest entry
      const latestEntry = entries.reduce((max, curr) => (!max || curr[0] > max[0] ? curr : max), undefined);
...
No repitas los datos que ya te mandé. No me pidas registrar nada.`;
  ...
  const generateWeeklyPDF = async () => {
    ...
    try {
      ...
      // ⚡ Bolt: Use O(N) reduce instead of O(N log N) sort to find latest date
...
function Coach({
  ...
  const contextSummary = React.useMemo(() => {
    ...
    // ⚡ Bolt: Use O(N) reduce instead of O(N log N) sort to find latest metrics
